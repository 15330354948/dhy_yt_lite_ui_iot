 ...
 * @LastEditTime: 2021-04-25 17:01:04
 * @LastEditors: Please set LastEditors
 * @Description: In User Settings Edit
 * @FilePath: \LH-UI\src\views\wel\index\dataAnalysis_right.vue
-->
<template>
  <div
    class="data_analysis_bottom_right"
    :class="{
      isHomePage_right: !isHomePage,
      data_analysis_right_full: viewLeftFull
    }">
    <div class="data_analysis_bottom_right_tem" v-for="item in moduleList.right" :key="item.data.name" v-show="item.data.templateUrl !== ''">
      <div class="borable leftup"></div>
      <div class="borable leftunder"></div>
      <div class="borable rightup"></div>
      <div class="borable rightunder"></div>
      <div class="data_analysis_bottom_right_tem_title"   @click="detailsSkip(item.data)" :style="{cursor:item.data.hasDetails?'pointer':'auto'}">
        <!-- icon图片 -->
        <span><img :src="require(`@/assets/img/icon/${item.data.icon}`)" alt="" /></span>
        <!-- title展示 -->
        <span>
          {{ item.data.moduleVisible ? monitorModStatus ? item.data.moduleTitle : item.data.moduleTitleChange : item.data.moduleTitle}}</span>
        <span><span></span><span></span></span>
      </div>
      <div class="data_analysis_bottom_right_tem_body">
...
export default {
  computed: mapGetters(["isDataAnalysis", "analysisDetails",  "monitorModStatus", "projectId", "viewConfig"]),
  data() {
    return {
        ...
        if(newConfig.panelCount){
          ...
          for (var key in viewConfigAll) {
            viewConfigAll[key].forEach((m) => {
              moduleright[key] || (moduleright[key] = []);
              moduleright[key].push({
                data: m,
...
.data_analysis_bottom_right.isHomePage_right {
  right: -26.88%;
}
</style>
