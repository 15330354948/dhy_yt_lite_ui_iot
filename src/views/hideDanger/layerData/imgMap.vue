  ...
  <div class="bc_cloud-tab">
    <div class="bc_label">
      <label slot="label">
        <div class="bc_cloud-label">
          <el-button type="primary" v-if="permissions.JCD_layer_imgMap_edit" size="mini" @click.native="layersSubmit">
      ...
      <el-table-column label="操作" align="center" width="180">
        <template slot-scope="scope">
          <el-button
            ...
            >绘制</el-button
          >
          <el-button size="mini" v-if="permissions.JCD_layer_imgMap_preview" type="text" @click="preview('yxt')"
            >预览</el-button
          >
          <el-button
            size="mini"
    ...
    >
      <div slot="title" class="dialog-title">
        <span class="title-text">监测点图层</span>
...
export default {
  ...
  watch: {
    disasterId: {
      immediate: true,
      handler(x, y) {
        if (x && this.moduleConfig.data.moduleTitle == "影像图") {
          this.getList();
        }
      },
    },
  ...
  methods: {
    getList() {
      //获取数据列表
      if (this.disasterId) {
        getImgMapList({ disasterId: this.disasterId }).then((res) => {
      ...
      if (!this.moduleConfig.data.moduleSave) {
        ...
        if (this.dangerAreaList.link != "" && arr.length > 0) {
          let layerDisasterData = {
            ...
            disasterImageMaps: this.dangerAreaList
          };
          addImgMap(layerDisasterData)
            .then((res) => {
      ...
      switch (val) {
        case "jxt":
          ...
          setTimeout((e) => {
            this.mapFunc.addJxt({
              name: "jxt",
              link: this.dangerAreaList.imageMap,
            });
            this.mapFunc.goView({
              longitude: this.disasterData.longitude,
              latitude: this.disasterData.latitude,
              height: 1600,
            });
          }, 200);
          break;
