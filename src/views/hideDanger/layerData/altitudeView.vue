...
export default {
  ...
  data() {
    return {
      ...
      moduleSave: false,
      dangerAreaList: [],
      disasterElevationPhotographyList: [
        {
          disasterId: 0,
          link: "",
          name: "",
  ...
  watch: {
    disasterId: {
      immediate: true,
      handler(x, y) {
        if (x && this.moduleConfig.data.moduleTitle == "高程") {
          this.getList();
        }
      },
    },
  ...
  methods: {
    getList() {
      if (this.disasterId) {
        getTiltAltitudeList({ disasterId: this.disasterId }).then((res) => {
    ...
    handleDelete(index, row) {
      ...
      this.$confirm("是否确认删除?", {
        ...
        const ids = [row.id];
        deleteTiltAltitude(ids).then((res) => {
      ...
      if (!this.moduleConfig.data.moduleSave) {
        ...
        if (this.dangerAreaList.link != "" && arr.length > 0) {
          let layerDisasterData = {
            ...
            disasterElevationPhotographyList:this.dangerAreaList
          };
          addTiltAltitude(layerDisasterData)
            .then((res) => {
