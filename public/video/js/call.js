(function (RongCall, dependencies) {
    /* 通话逻辑 */
    console.log(RongCall, dependencies);
    ...
    var commandEvents = {
        ...
        HungupMessage: function (message, context) {
            ...
            toast(reason);
            if (context.minUserList.length <= 1) document.getElementById('hungup').click();
        },
    ...
    //截取url地址上的参数
    function getUrlData(strName) {
        
        var urlStr = location.href;
        var index = urlStr.indexOf('?');
        var paramStr = urlStr.substr(index + 1);
        
        var paramArr = paramStr.split('&');
        for (var i = 0; i < paramArr.length; i++) {
            var param = paramArr[i].split("=");
            if (param[0].toUpperCase() == strName.toUpperCase()) {
                return param[1];
            }
        }
    }
    ...
    RongCall.call = Vue.component('call', {
        ...
        computed: {
            ...
            minUserList: function () {
                ...
                return context.userList.filter(function (user, i) {
                    return  i>0;
                });
            },
        },
        ...
        created: function () {
            var context = this;
            this.username = getUrlData('username');
            this.realname = decodeURI(getUrlData('realname'));
            this.selfUserId = getUrlData('phone');
            if(this.selfUserId){
                utils.login(this.selfUserId, this.realname).then(function (res) {
                    console.log(res);
                    if (res.code == 0) {
                        context.userInfo = res;
                        setting.token = res.token;
                        return RongCall.initIM(setting);
                    }
                }).then(function () {
                    RongCall.instance.auth = context.userInfo;
                }).catch(function () {
