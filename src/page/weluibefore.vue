  ...
  methods: {
    ...
    showChart(dataRec) {
      if (dataRec && JSON.stringify(dataRec) != "{}") {
        ...
        this.$refs.welChartRef.emptyOption();
      }
    },
    initEchart(dataRec) {
      var echartsKK = require("echarts");
      // 基于准备好的dom，初始化echarts实例
      var myChart = echartsKK.init(document.getElementById("welEchart"));
      if (dataRec && JSON.stringify(dataRec) != "{}") {
        // 绘制图表
        let optionChart = {
          legend: {
        ...
        var html =
          '<div style="text-align:center;"><img src="@/assets/img/nodata.png"  /></div>';
        document.getElementById("welEchart").innerHTML = html;
        document
          .getElementById("welEchart")
