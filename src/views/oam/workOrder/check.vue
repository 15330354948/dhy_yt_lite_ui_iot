    ...
    <basic-container>
      <avue-crud ref="crud" :page.sync="page" :data="tableData" :option="tableOption" v-model="tableObj"
        ...
        <template slot="menu" slot-scope="scope">
          ...
          <el-button v-if="scope.row.faultState == 4 && permissions['release_point_chexiao']" type="text" size="small"
            class="none-border" @click.stop="chexiao(scope.row, scope.index)">撤销
          </el-button>
        </template>
      </avue-crud>
    </basic-container>
...
<script>
  ...
  import {
    tableOption
  } from "@/const/crud/oam/workOrder/check";
    ...
    methods: {
      ...
      handleView(row, index) {
        infoOrder(row.id).then(res => {
          ...
          this.obj.revokeStatus = false;
          if (row.faultState == 5 || row.faultState == 4 || row.faultState == 0 || row.faultState == 1) {
            this.obj.shenhe = false;
          } else {
