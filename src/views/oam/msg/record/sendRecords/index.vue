...
import {sendRecord,sendPersonData,sendMsg } from '@/api/oamMsg/setting'
export default {
  computed: {
  ...
  methods: {
    ...
    xiafaClick() {
      if (
        this.formxiafa.personnelList.length != 0 &&
        this.formxiafa.content
      ) {
        this.dialogVisible = false;
        this.formxiafa.projectId = this.projectId;
        this.formxiafa.content+=" 请持续观察数据变化情况，加强人工巡查，做好安全措施！"
        this.formxiafa.issueType=1
        this.formxiafa.type=3
        sendMsg(this.formxiafa).then((v) => {
          if (v.data.code == 0) {
    ...
    getdata(num) {
      sendPersonData({projectId: num}).then(v=>{
        this.dataArr = v.data.data.records;
        const dataN = [];
        for (let i = 0; i < this.dataArr.length; i++) {
          dataN.push({
            key: this.dataArr[i].name + "(" + this.dataArr[i].telephone + ")",
            label: this.dataArr[i].name + "(" + this.dataArr[i].telephone + ")",
          });
        }
        this.data = dataN;
      })
    ...
    handleChange(value, direction, movedKeys) {
      if (value) {
        let arr = [];
        arr = value;
        this.formxiafa.personnelList = [];
        arr.map((v) => {
          if (v) {
            let b=v.substring(0,v.lastIndexOf("("));
            let a = v.substring(0, v.lastIndexOf(")"));
            let str = a.substring(a.lastIndexOf("(") + 1, a.length);
            this.formxiafa.personnelList.push({personName:b,telephone:str});
          }
        });
      } else {
        this.formxiafa.personnelList = [];
    ...
    handleFilter(form, done) {
      if(form.createTime&&form.createTime.length==2){
                form.beginDate =form.createTime[0]
                form.endDate =form.createTime[1]
