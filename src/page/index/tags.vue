<template>
  <div class="avue-tags" v-if="isMenu" @click="contextmenuFlag = false">
    <!-- tag盒子 -->
    <div
...
export default {
  ...
  created() {},
  mounted() {
    this.tagListNotData = this.tagList.filter((res) => {
          return res.label != "数据看板";
        });
    this.setActive();
  },
  watch: {
    ...
    tagList: {
      handler(val) {
        this.tagListNotData = val.filter((res) => {
          return res.label != "数据看板";
        });
      },
      deep: true,
  ...
  computed: {
    ...mapGetters(["isMenu", "tagWel", "tagList", "tag", "website"]),
    ...mapState({
